...
class general_yolov3(object):
    def __init__(self):
        ...
        self.detection = 'unhelmet'
        self.roi_x1 = 100 #入侵区域
        self.roi_y1 = 350
        self.roi_x2 = 400
        self.roi_y2 = 600
    ...
    def postprocess(self, img_cv2, outputs):
        ...
        #画出入侵区域
        cv2.rectangle(img_cv2, (self.roi_x1,self.roi_y1), (self.roi_x2, self.roi_y2), (0, 0, 255), 5)
    ...
    def vis_res(self, img_cv2, results):
        ...
        for result in results:
            ...
            #是否在入侵区域 如果是进行标记-
            if (left > self.roi_x1) & (top  > self.roi_y1) & (right < self.roi_x2) & (bottom < self.roi_y2):
                cv2.rectangle(img_cv2,(left, top),(right, bottom),(255, 178, 50), 3)
                ...
                cv2.rectangle(img_cv2,
                        ...
                        top + baseline), (255, 0, 0),cv2.FILLED)
                cv2.putText(img_cv2, label, (left, top),cv2.FONT_HERSHEY_SIMPLEX,0.75, (0, 0, 0), 1)
                #如果是需要检测的目标，输出
                if self.classes[class_id] == self.detection:
                    print('there is {}'.format(self.classes[class_id]))
        # 计算速率信息
...
if __name__ == '__main__':
    ...
    cap = cv2.VideoCapture('E:/project/helmet/站内.mp4')
    print(cap.isOpened())
