...
class Window(Frame):
    ...
    def model_intrude(self):
        ...
        try:
            print('zs/polygonvertex/boundaryIntrude/{}/channel{}/out'.format(self.zhanchang[self.zhanchange_index],
                                                                     self.channel_num))
            self.pub.send_msg(topic='zs/polygonvertex/boundaryIntrude/{}/channel{}/out'.format(self.zhanchang[self.zhanchange_index],self.channel_num),
                          msg=msg)
            tkinter.messagebox.showinfo('提示', '发送成功')
        except:
            tkinter.messagebox.showerror('错误', '发送失败')
