...
import publish
import json
from ptz_move import continuous_move,move_up,move_down,move_left,move_right
...
class Window(Frame):
    def __init__(self, master=None,config=None,config_path=None):
        ...
        config_.add_command(label="model", command=self.model_config)
        ...
        #云台按钮
        # UP
        button_up = tkinter.Button(self.master, text="UP", fg="white", bg='black', command=self.up)
        button_up.place(x=70,y=300,width=50)
        # DOWN
        button_down = tkinter.Button(self.master, text="DOWN", fg="white", bg='black', command=self.down)
        button_down.place(x=70, y=350,width=50)
        # RIGHT
        button_right = tkinter.Button(self.master, text="RIGHT", fg="white", bg='black', command=self.right)
        button_right.place(x=130, y=350, width=50)
        # LEFT
        button_left = tkinter.Button(self.master, text="LEFT", fg="white", bg='black', command=self.left)
        button_left.place(x=10, y=350, width=50)
    ...
    def channel_select(self,event):
        ...
        print(self.ip)
        self.ptz,self.request = continuous_move(self.ip)
        #双击左键start
        self.show_image()
    ...
    def model_config(self):
        self.modelname = filedialog.askopenfilename()


    def up(self):
        print(self.ip)
        move_up(self.ptz, self.request)

    def down(self):
        move_down(self.ptz, self.request)

    def right(self):
        move_right(self.ptz, self.request)

    def left(self):
        move_left(self.ptz, self.request)
    ...
    def cap_read(self):
        ...
        cap = cv2.VideoCapture(
            'rtsp://admin:admin12345@{}:554'.format(self.ip))
        if cap.isOpened():
            return cap
        else:
            return None
